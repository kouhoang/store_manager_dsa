...
public class AccountForm extends javax.swing.JInternalFrame {
    ...
    public AccountForm() {
        ...
        initTable();
        loadDataToTable(Run.AccountData);
        
    }

    public final void initTable() {
        tblModel = new DefaultTableModel();
        String[] headerTbl = new String[]{"Tên tài khoản", "Tên đăng nhập", "Mật khẩu", "Vai trò"};
        ...
        try {
            ...
            for (Account i : acc) {
                tblModel.addRow(new Object[]{
                    i.getFullName(), i.getUser(), i.getPassword(), i.getRole()
                });
            }
        } catch (Exception e) {
        }
 ...
 public int getAccountSelect() {
    ...
    private void initComponents() {
        ...
        jPanel2.add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(190, 10, 980, 90));
 ...
 loadDataToTable(Run.AccountData);        // TODO add your handling code here:
  ...
  String luachon = (String) cbxLuachon.getSelectedItem();
        ...
        switch (luachon) {
            ...
            case "Tên tài khoản":
                ...
                break;
            case "Tên đăng nhập":
                result = SearchAccount.getInstance().searchUserNames(searchContent);
                break;
            case "Vai trò":
   ...
   // TODO add your handling code here:
        if (tblAccount.getSelectedRow() == -1) {
            ...
            int select = getAccountSelect();
            if (Run.AccountData.get(select).getRole().equals("admin")) {
   ...
   String luaChon = cbxLuachon.getSelectedItem().toString();
        ...
        ArrayList<Account> result = searchFn(luaChon, content);
        loadDataToTableSearch(result);        // TODO add your handling code here:
    }//GEN-LAST:event_cbxLuachonPropertyChange

   
 public ArrayList<Account> searchFn(String luaChon, String content) {
        ArrayList<Account> result = new ArrayList<>();
        SearchAccount searchPr = new SearchAccount();
        switch (luaChon) {
            case "Tất cả":
                result = searchPr.searchTatCaAcc(content);
                break;
            case "Mã máy":
                result = searchPr.searchFullName(content);
                break;
            case "Tên máy":
                result = searchPr.searchUserNames(content);
                break;
            case "Số lượng":
                result = searchPr.searchRole(content);
                break;
            case "Đơn giá":
                result = searchPr.searchPassword(content);
                break;
                 
        }
        return result;
 }
 public void loadDataToTableSearch(ArrayList<Account> result) {
        try {
            tblModel.setRowCount(0);
